...
## GitHub Marketplace Action (Production Deploy)

This repository now exposes a reusable GitHub Action at [action.yml](action.yml) that wraps [deploy-production.ps1](deploy-production.ps1).

### What it does

- Provisions or reuses Azure RG, ACR, and AKS
- Builds and pushes service images
- Applies production Kubernetes manifests
- Configures ingress prerequisites used by production deployment

### Quick usage in another workflow
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy DMD Cloud
        uses: taoufikmohamed/diff-dmd@v1
        with:
...
1. Ensure repository is public and includes [action.yml](action.yml).
2. Create a semantic tag and push it:
```bash
git tag -a v1.0.0 -m "Marketplace release"
git push origin v1.0.0
```
3. Run/verify [publish-action.yml](.github/workflows/publish-action.yml) (auto-runs on v* tags).
4. In GitHub: repository -> Releases -> verify release for the tag.
5. In GitHub: repository -> Settings -> Actions -> General -> enable Marketplace listing, then publish.
6. Add moving major tag for stable adoption:
```bash
git tag -f v1
...
**For CI cost/carbon optimization and deployment secret setup, see [Green Workflows and Required Secrets](#green-workflows-and-required-secrets).**

## Green Workflows and Required Secrets

The repository now includes green/FinOps-focused GitHub Actions workflows to reduce unnecessary CI runs, improve image efficiency, and support safer Azure deployments.

### Workflows

| Workflow | File | Purpose |
|----------|------|---------|
| CI/CD Pipeline | `.github/workflows/ci-cd.yml` | Path-filtered CI (only changed services), dependency/image caching, Terraform validation |
| Deploy to Production | `.github/workflows/deploy-production.yml` | Manual AKS deployment using Azure OIDC and rollout checks |
| Publish GitHub Action | `.github/workflows/publish-action.yml` | Creates a release when a version tag (`v*`) is pushed for Marketplace publishing |
| Green Policy & FinOps | `.github/workflows/green-policy.yml` | Weekly image audit (Trivy), repo hygiene, and large-file/stale-branch checks |
| Deploy Landing Page | `.github/workflows/pages.yml` | Publishes `docs/` to GitHub Pages |
...
|--------|---------|-------------|
| `AZURE_CLIENT_ID` | Deploy to Production | Azure AD app/client ID for OIDC login |
| `AZURE_TENANT_ID` | Deploy to Production | Azure tenant ID |
| `AZURE_SUBSCRIPTION_ID` | Deploy to Production | Azure subscription ID |
| `AKS_RESOURCE_GROUP` | Deploy to Production | Resource group containing AKS |
| `AKS_CLUSTER_NAME` | Deploy to Production | AKS cluster name |
| `ACR_LOGIN_SERVER` | Deploy to Production | ACR login server (for example: `myacr.azurecr.io`) |

### Runtime Secret (Kubernetes)

This is required by the in-cluster AI service (not a GitHub secret):

| Secret | Location | Description |
|--------|----------|-------------|
| `DEEPSEEK_API_KEY` | Kubernetes secret `ai-service-secrets` | API key used by `ai_service` to generate workflow YAML |
| `GITHUB_TOKEN` | Kubernetes secret `webhook-service-secrets` (optional) | GitHub token used by `webhook_service` to fetch commit diff from GitHub Compare API when webhook payload has no `diff` field |
...
**View all logs:**
```powershell
kubectl logs -l app=webhook-service -n dmd-production --all-containers=true --tail=100
kubectl logs -l app=ai-service -n dmd-production --all-containers=true --tail=100
```
